...
const rsvpCounts = computed(() => {
  const data =
    rsvpsWithPhone.value.length > 0 ? rsvpsWithPhone.value : props.rsvps;
  const inCount = data.filter((r) => r.status === 'IN' && r.phone).length;
  const waitlistCount = data.filter(
    (r) => r.status === 'WAITLIST' && r.phone
  ).length;
...
const audienceOptions = computed(() => [
  ...
  {
    value: 'in_waitlist' as Audience,
    label: 'In + Wait',
    count: rsvpCounts.value.in_waitlist,
  },
...
const selectedRsvps = computed(() => {
  ...
  switch (audience.value) {
    ...
    case 'in_waitlist':
      return data.filter(
        (r) => (r.status === 'IN' || r.status === 'WAITLIST') && r.phone
      );
    case 'all':
        ...
        <template v-else>
          ...
          <div>
            ...
            <p class="text-xs text-gray-400 mt-2">
              Sending to {{ phoneNumbers.length }}
              {{ phoneNumbers.length === 1 ? 'person' : 'people' }}
            </p>
          </div>
        </template>
      </div>
