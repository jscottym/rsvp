...
watch(() => props.initialData, (newData) => {
  if (newData) {
    ...
    if (newData.description !== undefined) form.description = newData.description
    if (newData.allowSharing !== undefined) form.allowSharing = newData.allowSharing
  }
}, { deep: true })
...
<template>
  <div class="flex flex-col h-full">
    <div class="flex-1 overflow-y-auto" :class="{ 'pb-24': !inline, 'pb-4': inline }">
      <!-- Location -->
      <section class="mb-6">
    ...
    <div
      :class="[
        inline
          ? 'mt-4 px-4'
          : 'fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-gray-50 via-gray-50 dark:from-gray-950 dark:via-gray-950 to-transparent pt-8 z-40'
      ]"
    >
      <div :class="{ 'max-w-lg mx-auto': !inline }">
        ...
        >
          ...
          <template v-else>
            <span>{{ submitLabel || `Create ${form.maxPlayers}-player Game` }}</span>
